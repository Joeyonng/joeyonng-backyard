export const rgba = (rgb: string, opacity: number): string => {
  const numbers = rgb.match(/\d+/g);
  if (!numbers || numbers.length < 3) return `rgba(0, 0, 0, ${opacity})`;
  return `rgba(${numbers[0]}, ${numbers[1]}, ${numbers[2]}, ${opacity})`;
};

export const rmPx = (px: string): number => {
...
export const glassBackground = (): { background: string; backdropFilter: string } => ({
